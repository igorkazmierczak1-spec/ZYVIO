...
export function Logo({ onClick }: { onClick?: () => void }) {
  ...
  return <button type="button" className="brand-button" aria-label="Go to ZYVIO home" onClick={onClick}>{content}</button>;
}
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const secondaryItems = [
    ...
    { href: "/ai", label: "ZYVIO AI", icon: Bot },
...
function BattleDetailPage() {
  ...
  const shareBattle = async () => {
    const shareData = { title: data.title, text: `Join this 1v1 battle on ZYVIO: ${data.title}`, url: window.location.href };
    try {
      if (navigator.share) {
      ...
      <section className="panel" style={{ padding: '24px', marginBottom: '14px', border: '1px solid var(--lime)', background: '#f1f9d4' }}>
        <div className="panel-heading" style={{ margin: 0, marginBottom: '16px' }}>
          <div>
           ...
           <h2 style={{ color: '#293b09' }}>{currentPlan === "PREMIUM_PRO" ? "ZYVIO Premium Pro aktywne" : "ZYVIO Premium aktywne"}</h2>
...
function PublicLanding() {
  const [, navigate] = useLocation();
  return <main className="auth-landing"><div className="auth-landing-panel"><Logo /><span className="eyebrow">Global creative competition</span><h1>Make your move.<br /><em>Own your ZYVIO.</em></h1><p>Join creative battles, earn XP, and build a profile that reflects what you can do.</p><div className="auth-actions"><Button onClick={() => navigate("/sign-up")}>Create account <ArrowUpRight /></Button><Button variant="secondary" onClick={() => navigate("/sign-in")}>Sign in</Button></div><a className="auth-privacy-link" href={`${basePath}/privacy`}>Polityka prywatności <ArrowUpRight /></a></div><div className="auth-visual"><div className="auth-orbit orbit-a" /><div className="auth-orbit orbit-b" /><div className="auth-orbit orbit-c" /><div className="auth-visual-mark"><Swords /><span>ZYVIO</span></div></div></main>;
}
