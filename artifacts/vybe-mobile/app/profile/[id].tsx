...
export default function SocialProfileScreen() {
  ...
  const data = profile.data;
   return <AppScreen><BackButton /><Card accent={colors.primary}><View style={{ alignItems: 'center', gap: 8 }}><Avatar name={data.displayName} uri={data.avatarUrl} size={78} /><Text style={[uiStyles.title, { color: colors.foreground, fontSize: 23 }]}>{data.displayName}</Text><Text style={{ color: colors.mutedForeground }}>@{data.username} · {data.country}</Text><Text style={{ color: colors.foreground, textAlign: 'center' }}>{data.bio}</Text></View>
