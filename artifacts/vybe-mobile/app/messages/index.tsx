...
export default function MessagesScreen() {
  ...
  return <AppScreen refreshing={conversations.isFetching} onRefresh={() => void conversations.refetch()}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><BackButton /><Header eyebrow="VYBE / PRIVATE" title="Wiadomości." /></View>
    {conversations.isLoading ? <LoadingState /> : conversations.isError ? <ErrorState onRetry={() => void conversations.refetch()} /> : !conversations.data?.length ? <EmptyState title="Brak rozmów" body="Otwórz profil twórcy, aby rozpocząć prywatną rozmowę." icon="paper-plane-outline" /> : conversations.data.map((conversation) => <Pressable key={conversation.id} onPress={() => router.push(`/messages/${conversation.id}`)}><Card><View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><Avatar name={conversation.otherParticipant.displayName} /><View style={{ flex: 1 }}><Text style={{ color: colors.foreground, fontFamily: 'Inter_700Bold' }}>{conversation.otherParticipant.displayName}</Text><Text style={{ color: colors.mutedForeground }} numberOfLines={1}>{conversation.lastMessage?.body ?? 'Nowa rozmowa'}</Text></View><Text style={{ color: colors.primary }}>›</Text></View></Card></Pressable>)}
  </AppScreen>;
}