...
export default function ProfileScreen() {
  ...
  return <AppScreen><Header eyebrow="ZYVIO / PROFILE" title="Twoja legenda." right={<Avatar name={data.displayName} size={54} />} />
    <Card accent={colors.accent}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}><Avatar name={data.displayName} size={62} /><View style={{ flex: 1, gap: 4 }}><Text style={[uiStyles.title, { color: colors.foreground, fontSize: 21 }]}>{data.displayName}</Text><Text style={[uiStyles.subtitle, { color: colors.mutedForeground }]}>@{data.username} · {data.country}</Text>{data.role === 'ADMIN' ? <Text style={{ color: colors.destructive, fontSize: 10, letterSpacing: 1.2, fontFamily: 'Inter_700Bold' }}>ADMIN</Text> : null}</View></View><Text style={[uiStyles.subtitle, { color: colors.foreground }]}>{data.bio || 'Tworzę swój ZYVIO krok po kroku.'}</Text></Card>
